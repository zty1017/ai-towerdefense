#!/usr/bin/env python3
"""Validate a MapPathGeometryReport v0.1 JSON file."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Validate a MapPathGeometryReport v0.1 JSON file."
    )
    parser.add_argument("report", help="Map path geometry report JSON path.")
    ...
    try:
        ...
        print("INVALID MapPathGeometryReport")
    ...
    if not isinstance(schema, dict):
        schema = None
    errors = mpg.validate_geometry_report(report, schema)
    if errors:
        print("INVALID MapPathGeometryReport")
        for error in errors:
            print(f"- {error}")
