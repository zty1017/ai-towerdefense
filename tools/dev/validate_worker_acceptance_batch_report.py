...
SCHEMA_VERSION = "worker_acceptance_batch_run_report.v0.1"
VALID_STATUSES = {"passed", "dry_run", "failed"}
...
def validate_report(
    ...
    *,
    ...
    actual_failed_count = sum(1 for pack in packs if isinstance(pack, dict) and pack.get("status") == "failed")
    actual_passed_count = sum(1 for pack in packs if isinstance(pack, dict) and pack.get("status") == "passed")
    ...
    for index, pack in enumerate(packs):
        ...
        require(pack.get("status") in VALID_STATUSES, f"packs[{index}].status invalid")
