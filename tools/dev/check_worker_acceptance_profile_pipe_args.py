#!/usr/bin/env python3
"""Smoke-check profile runner parsing of safe regex pipe arguments."""
...
REPORT_SCHEMA_VERSION = "worker_acceptance_profile_pipe_args_smoke.v0.1"
DEFAULT_OUTPUT = Path("/tmp/worker_acceptance_profile_pipe_args_smoke.v0.1.json")
...
def run_smoke() -> dict[str, Any]:
    safe = parse_command('rg -n "alpha|beta" docs README.md')
    assert safe.argv == ["rg", "-n", "alpha|beta", "docs", "README.md"]
    ...
    return {
        ...
        "status": "passed",
        "safe_regex_pipe_argv": safe.argv,
        "rejected": rejected,
        "safety_summary": {
...
def main() -> int:
    ...
    try:
        ...
        print(f"worker acceptance pipe args smoke failed: {exc}", file=sys.stderr)
        return 1
    print(f"worker acceptance pipe args smoke passed: {output}")
    return 0
