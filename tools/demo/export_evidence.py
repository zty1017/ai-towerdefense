...
PATHS = {
    ...
    "provider_runner_handoff_outbox_consumer_task_pack": ROOT
    / "examples/worker_task_packs/p1b_provider_runner_handoff_outbox_consumer.v0.1.json",
    "provider_runner_handoff_outbox_import_smoke_task_pack": ROOT
    ...
    "provider_runner_handoff_outbox_consumer": ROOT
    / "tools/dev/run_provider_adapter_runner_handoff_outbox.py",
    "provider_runner_handoff_outbox_import_smoke": ROOT
    ...
    "map_runtime_activation_authorization_report": ROOT
    ...
    "map_decoration_zone_policy": ROOT
    ...
    "controlled_map_candidate_artifact_import_report": ROOT
...
STATIC_VALIDATION_COMMANDS = [
    ...
    {
        "name": "map_runtime_activation_authorization",
        "command": [
            "python3",
            "tools/media/validate_map_runtime_activation_authorization_report.py",
            "examples/review_packs/map_runtime_activation_authorization_report.v0.1.json",
        ],
    },
    {
    ...
    {
        "name": "map_decoration_zone_policy",
        "command": [
            "python3",
    ...
    {
        "name": "controlled_map_candidate_artifact_import_report",
        "command": [
            "python3",
...
def map_runtime_activation_gate_summary(report: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "authorization_status_counts": as_obj(summary.get("authorization_status_counts")),
        "authorization_approved_count": summary.get("authorization_approved_count"),
        "frontend_v02_contract_status": summary.get("frontend_v02_contract_status"),
        ...
        "decision_samples": [
            {
                ...
                "frontend_v02_contract_status": as_obj(decision.get("safety")).get(
...
def map_runtime_activation_authorization_summary(report: dict[str, Any]) -> dict[str, Any]:
    summary = as_obj(report.get("summary"))
    safety = as_obj(report.get("safety_summary"))
    nodes = [node for node in as_list(report.get("nodes")) if isinstance(node, dict)]
    return {
        ...
        "authorization_status_counts": as_obj(summary.get("authorization_status_counts")),
        "activation_authorized_for_gate_count": summary.get(
            "activation_authorized_for_gate_count"
        ),
        "safety": {
            "reads_env_file": safety.get("reads_env_file"),
            "provider_call_count_by_report": safety.get(
        ...
        "node_samples": [
            {
                "node_id": node.get("node_id"),
                "authorization_status": node.get("authorization_status"),
                "authorization_decision": node.get("authorization_decision"),
                "activation_authorized_for_gate": node.get(
                    "activation_authorized_for_gate"
                ),
                "to_package_id": as_obj(node.get("target_candidate")).get(
                    "to_package_id"
                ),
            }
            for node in nodes[:MAX_SAMPLE_ITEMS]
...
def map_decoration_zone_policy_summary(policy: dict[str, Any]) -> dict[str, Any]:
    summary = as_obj(policy.get("summary"))
    source_policy = as_obj(policy.get("source_policy"))
    maps = [item for item in as_list(policy.get("maps")) if isinstance(item, dict)]
    return {
        "schema_version": policy.get("schema_version"),
        "policy_id": policy.get("policy_id"),
        "map_count": summary.get("map_count"),
        "reserved_zone_count": summary.get("reserved_zone_count"),
        "allowed_decoration_zone_count": summary.get(
            "allowed_decoration_zone_count"
        ),
        "source_schema_versions": as_list(summary.get("source_schema_versions")),
        "strong_semantic_policy": summary.get("strong_semantic_policy"),
        "source_policy": source_policy,
        "usage_policy": as_list(policy.get("usage_policy")),
        "runtime_fact_source": source_policy.get("runtime_fact_source"),
        "player_default_runtime": source_policy.get("player_default_runtime"),
        "may_modify_map_runtime_package": source_policy.get(
            "may_modify_map_runtime_package"
        ),
        "provider_call_count": source_policy.get("provider_call_count"),
        "map_samples": [
            {
                "map_id": item.get("map_id"),
                "node_id": as_obj(item.get("source_package")).get("node_id"),
                "source_package_id": as_obj(item.get("source_package")).get(
                    "package_id"
                ),
                "source_schema_version": as_obj(item.get("source_package")).get(
                    "schema_version"
                ),
                "reserved_zone_count": len(as_list(item.get("reserved_zones"))),
                "allowed_decoration_zone_count": len(
                    as_list(item.get("allowed_decoration_zones"))
                ),
                "reserved_zone_type_counts": dict(
                    sorted(
                        Counter(
                            str(as_obj(zone).get("zone_type") or "unknown")
                            for zone in as_list(item.get("reserved_zones"))
                        ).items()
                    )
                ),
                "allowed_zone_type_counts": dict(
                    sorted(
                        Counter(
                            str(as_obj(zone).get("zone_type") or "unknown")
                            for zone in as_list(item.get("allowed_decoration_zones"))
                        ).items()
                    )
                ),
                "validation_status": as_obj(item.get("validation_report")).get(
                    "status"
                ),
                "warning_count": as_obj(item.get("validation_report")).get(
                    "warning_count"
                ),
            }
            for item in maps[:5]
...
def controlled_map_candidate_artifact_import_summary(report: dict[str, Any]) -> dict[str, Any]:
    summary = as_obj(report.get("summary"))
    items = [item for item in as_list(report.get("items")) if isinstance(item, dict)]
    return {
        "schema_version": report.get("schema_version"),
        "report_id": report.get("report_id"),
        "status": report.get("status"),
        "copy_files": report.get("copy_files"),
        "request_count": summary.get("request_count"),
        "awaiting_count": summary.get("awaiting_count"),
        "imported_count": summary.get("imported_count"),
        "invalid_count": summary.get("invalid_count"),
        "provider_call_count": summary.get("provider_call_count"),
        "runtime_mutation_count": summary.get("runtime_mutation_count"),
        "published_visual_layer_write_count": summary.get("published_visual_layer_write_count"),
        "map_runtime_package_write_count": summary.get("map_runtime_package_write_count"),
        "status_counts": as_obj(summary.get("status_counts")),
        "safety_summary": as_obj(report.get("safety_summary")),
        "source_file": report.get("import_plan_path"),
        "validation_commands": as_list(as_obj(report.get("validation")).get("commands")),
        "item_samples": [
            {
                "node_id": item.get("node_id"),
                ...
                "source_png_path": item.get("source_png_path"),
                "target_candidate_path": item.get("target_candidate_path"),
...
def map_runtime_v02_activation_contract_plan_summary(report: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "frontend_post_activation_evidence_required_count": summary.get(
...
def collect_provider_runner_handoff_summary() -> dict[str, Any]:
    ...
    outbox_consumer_task = load_json(
        PATHS["provider_runner_handoff_outbox_consumer_task_pack"]
    )
    outbox_import_smoke_task = load_json(
        PATHS["provider_runner_handoff_outbox_import_smoke_task_pack"]
    )
    ...
    return {
        ...
        "outbox_consumer_status": "local_dry_boundary_consumer_ready",
        "outbox_import_pipeline_status": "local_consume_import_prefetch_smoke_ready",
        "export_endpoint": (
        ...
        "outbox_consumer_evidence": {
            "report_schema_version": (
                "provider_adapter_runner_handoff_outbox_execution_report.v0.1"
            ),
            "supported_modes": ["fixture", "video"],
            "imports_to_backend": False,
            "staging_performed": False,
            "promotion_performed": False,
            "runtime_activation_allowed": False,
            "live_provider_execution_supported": False,
        },
        "outbox_import_pipeline_evidence": {
            "tool": "tools/dev/check_provider_runner_handoff_outbox_import_pipeline.py",
            "report_schema_version": (
                "provider_runner_handoff_outbox_import_pipeline_report.v0.1"
            ),
            "transport": "temporary_local_uvicorn_http",
            "consumer_report_schema_version": (
                "provider_adapter_runner_handoff_outbox_execution_report.v0.1"
            ),
            "expected_runner_handoff_count": 2,
            "expected_imported_count": 2,
            "expected_prefetch_status": "review_only_envelope_ready",
            "imports_to_temporary_backend_ledger": True,
            "fixture_boundary_only": True,
            "staging_performed": False,
            "promotion_performed": False,
        ...
        "tools": [
            file_ref(
                PATHS["provider_runner_handoff_outbox_consumer"],
                "outbox_consumer_tool",
            ),
            file_ref(
                PATHS["provider_runner_handoff_outbox_import_smoke"],
                "outbox_import_pipeline_smoke_tool",
            ),
        ],
        "acceptance_commands": sorted(
            ...
            | set(as_list(outbox_consumer_task.get("acceptance_commands")))
            | set(as_list(outbox_import_smoke_task.get("acceptance_commands")))
...
def collect_assets_and_media(
    ...
    map_decoration_zone_policy: dict[str, Any],
    map_controlled_regeneration_request_pack: dict[str, Any],
    controlled_map_candidate_artifact_import_report: dict[str, Any],
    ...
    return {
        ...
        "map_visual_reference": {
            ...
            "map_decoration_zone_policy": map_decoration_zone_policy_summary(
                map_decoration_zone_policy
            ),
            ...
            "controlled_candidate_artifact_import": controlled_map_candidate_artifact_import_summary(
                controlled_map_candidate_artifact_import_report
            ),
...
def collect_source_files() -> list[dict[str, Any]]:
    source_paths = [
        ...
        (
            ...
            PATHS["map_runtime_activation_authorization_report"],
        ),
        (
        ...
        ("map_decoration_zone_policy", PATHS["map_decoration_zone_policy"]),
        ...
        (
            "controlled_map_candidate_artifact_import_report",
            PATHS["controlled_map_candidate_artifact_import_report"],
        ),
        (
            "controlled_map_candidate_generation_run",
...
def build_evidence(
    ...
    map_runtime_activation_authorization_report = load_json(
        PATHS["map_runtime_activation_authorization_report"]
    )
    ...
    map_decoration_zone_policy = load_json(PATHS["map_decoration_zone_policy"])
    ...
    controlled_map_candidate_artifact_import_report = load_json(
        PATHS["controlled_map_candidate_artifact_import_report"]
    )
    ...
    evidence = {
        ...
        "map_runtime_activation_authorization": map_runtime_activation_authorization_summary(
            map_runtime_activation_authorization_report
        ),
    ...
    decoration_policy = as_obj(
        as_obj(as_obj(evidence.get("assets_and_media")).get("map_visual_reference")).get(
            "map_decoration_zone_policy"
        )
    )
    if decoration_policy.get("runtime_fact_source") is not False:
        raise ValueError("map decoration zone policy runtime_fact_source must be false")
    if decoration_policy.get("provider_call_count") != 0:
        raise ValueError("map decoration zone policy provider_call_count must be 0")
    if decoration_policy.get("player_default_runtime") is not False:
        raise ValueError("map decoration zone policy player_default_runtime must be false")
    if decoration_policy.get("may_modify_map_runtime_package") is not False:
        raise ValueError(
            "map decoration zone policy may_modify_map_runtime_package must be false"
        )
...
def render_summary_markdown(evidence: dict[str, Any]) -> str:
    ...
    map_runtime_activation_authorization = as_obj(
    ...
    map_decoration_zone_policy = as_obj(
        as_obj(as_obj(evidence.get("assets_and_media")).get("map_visual_reference")).get(
            "map_decoration_zone_policy"
        )
    )
    ...
    controlled_candidate_artifact_import = as_obj(
        as_obj(as_obj(evidence.get("assets_and_media")).get("map_visual_reference")).get(
            "controlled_candidate_artifact_import"
        )
    )
    ...
    lines = [
        ...
        f"- runner handoff：`{provider_runner_handoff.get('status')}`，outbox consumer：`{provider_runner_handoff.get('outbox_consumer_status')}`，outbox import smoke：`{provider_runner_handoff.get('outbox_import_pipeline_status')}`，video 边界：`{provider_runner_handoff.get('video_boundary_status')}`，roundtrip cache：`{provider_runner_handoff_roundtrip.get('expected_cache_status')}`，runtime 激活：`{provider_runner_handoff_roundtrip.get('runtime_activation_allowed')}`",
        ...
        f"- MapDecorationZonePolicy：地图 `{map_decoration_zone_policy.get('map_count')}`，强语义保护区 `{map_decoration_zone_policy.get('reserved_zone_count')}`，可装饰区 `{map_decoration_zone_policy.get('allowed_decoration_zone_count')}`，runtime fact source `{map_decoration_zone_policy.get('runtime_fact_source')}`，provider calls `{map_decoration_zone_policy.get('provider_call_count')}`，可修改 MapRuntimePackage `{map_decoration_zone_policy.get('may_modify_map_runtime_package')}`",
        ...
        f"- 地图受控候选本地 PNG 导入：`{controlled_candidate_artifact_import.get('status')}`，awaiting `{controlled_candidate_artifact_import.get('awaiting_count')}`，imported `{controlled_candidate_artifact_import.get('imported_count')}`，provider calls `{controlled_candidate_artifact_import.get('provider_call_count')}`，校验命令 `{controlled_candidate_artifact_import.get('validation_commands')}`",
  ...
  <main>
    <section>
      ...
      <div class="grid">
        ...
        <article class="card">
          <div class="eyebrow">Map Runtime 授权</div>
          <div class="metric">{html_escape(map_runtime_activation_authorization.get("status"))}</div>
          <p class="muted">approved {html_escape(map_runtime_activation_authorization.get("approved_count"))}；pending {html_escape(map_runtime_activation_authorization.get("pending_count"))}；runtime 修改 {html_escape(as_obj(map_runtime_activation_authorization.get("safety")).get("default_runtime_mutation_performed"))}。</p>
        </article>
        <article class="card">
        ...
        <article class="card">
          <div class="eyebrow">MapDecorationZonePolicy</div>
          <div class="metric">{html_escape(map_decoration_zone_policy.get("map_count"))}</div>
          <p class="muted">保护区 {html_escape(map_decoration_zone_policy.get("reserved_zone_count"))}；可装饰区 {html_escape(map_decoration_zone_policy.get("allowed_decoration_zone_count"))}；runtime fact source {html_escape(map_decoration_zone_policy.get("runtime_fact_source"))}；provider calls {html_escape(map_decoration_zone_policy.get("provider_call_count"))}。</p>
        </article>
        <article class="card">
        ...
        <article class="card">
          <div class="eyebrow">受控 PNG 导入</div>
          <div class="metric">{html_escape(controlled_candidate_artifact_import.get("status"))}</div>
          <p class="muted">awaiting：{html_escape(controlled_candidate_artifact_import.get("awaiting_count"))}；imported：{html_escape(controlled_candidate_artifact_import.get("imported_count"))}；provider calls：{html_escape(controlled_candidate_artifact_import.get("provider_call_count"))}。</p>
        </article>
        <article class="card">
    ...
    <section>
      ...
      <p>runner handoff：<code>{html_escape(provider_runner_handoff.get("status"))}</code>；outbox consumer：<code>{html_escape(provider_runner_handoff.get("outbox_consumer_status"))}</code>；outbox import smoke：<code>{html_escape(provider_runner_handoff.get("outbox_import_pipeline_status"))}</code>；video 边界：<code>{html_escape(provider_runner_handoff.get("video_boundary_status"))}</code>；roundtrip cache：<code>{html_escape(provider_runner_handoff_roundtrip.get("expected_cache_status"))}</code>；runtime 激活：<code>{html_escape(provider_runner_handoff_roundtrip.get("runtime_activation_allowed"))}</code></p>
