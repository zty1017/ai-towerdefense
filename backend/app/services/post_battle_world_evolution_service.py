...
def evolve_world(
    *,
    ...
    fallback = {"applied": False, "state": deterministic_state}
    if not _enabled():
        return fallback
    ...
    try:
        response = _request_provider_response(
            messages,
            timeout=_bounded_env_int("AI_TD_WORLD_EVOLUTION_TIMEOUT", 8, 1, 30),
            max_tokens=_bounded_env_int("AI_TD_WORLD_EVOLUTION_MAX_TOKENS", 4096, 512, 8192),
        )
        delta = adapter.extract_json(adapter.extract_content_from_response(response))
        if not isinstance(delta, dict):
            return fallback
        ...
        if semantic_validator.validate_world_delta_semantics(
            delta, deterministic_state, registry
        ):
            return fallback
        if _controlled_policy_errors(delta, deterministic_state):
            return fallback
    ...
    return {
        ...
        "state": next_state,
        "delta": delta,
        "projection": _projection_fields(delta),
    }
