...
from .generation_scheduler_runtime_activation_authorization_builders import (  # noqa: E402
...
def get_generation_prefetch_cache(session_id: str) -> dict[str, Any]:
    ...
    ledger_items = _load_generation_artifact_ledger_items(session_id, run_id)
    return _build_generation_prefetch_cache_payload(
        session_id,
        latest_run,
        queue_items,
        ledger_items,
...
def _runtime_activation_artifact_report_ref(
    item: dict[str, Any],
) -> dict[str, Any] | None:
    refs = item.get("refs")
    if not isinstance(refs, dict):
        return None
    report_ref = refs.get(RUNTIME_ARTIFACT_BUILD_REPORT_LEDGER_KIND)
    return report_ref if isinstance(report_ref, dict) else None
...
def record_generation_runtime_activation_authorization(
    ...
    safe_metadata = metadata if isinstance(metadata, dict) else {}
    worker_id = str(
        safe_metadata.get("worker_id")
        or "generation_runtime_activation_authorization_recorder"
    )
    note = safe_metadata.get("note")
    decision = _normalize_runtime_activation_decision(
        safe_metadata.get("activation_decision")
    )
    latest_run = _load_latest_generation_schedule_run(session_id)
    if latest_run is None:
        raise InvalidQueueTransitionError(
            "generation schedule run is required before runtime activation authorization"
        )
    run_id = str(latest_run.get("run_id"))
    requested_schedule_item_id = _requested_schedule_item_id(safe_metadata)
    prefetch_payload = get_generation_prefetch_cache(session_id)
    ...
    selected_item: dict[str, Any] | None = None
    selected_report_ref: dict[str, Any] | None = None
    for item in prefetch_items:
        if not isinstance(item, dict):
            continue
        schedule_item_id = str(item.get("schedule_item_id") or "")
        if requested_schedule_item_id and schedule_item_id != requested_schedule_item_id:
            continue
        report_ref = _runtime_activation_artifact_report_ref(item)
        if report_ref is None:
            continue
        selected_item = item
        selected_report_ref = report_ref
        break

    if selected_item is None or selected_report_ref is None:
        if requested_schedule_item_id:
            raise InvalidQueueTransitionError(
                "no runtime artifact build report for "
                f"schedule item {requested_schedule_item_id}"
            )
        raise InvalidQueueTransitionError(
            "no runtime artifact build report is available"
        )

    schedule_item_id = str(selected_item.get("schedule_item_id") or "")
    queue_row = _load_generation_queue_item_row(session_id, schedule_item_id)
    queue_item = queue_row["payload"]
    authorization = _build_runtime_activation_authorization(
        session_id=session_id,
        latest_run=latest_run,
        queue_item=queue_item,
        runtime_artifact_report_ref=selected_report_ref,
        decision=decision,
        ts=ts,
        worker_id=worker_id,
        note=str(note) if note is not None else None,
    )
    compact = _compact_runtime_activation_authorization(authorization)
    ledger_entry = _build_artifact_ledger_payload(
        session_id=session_id,
        artifact_kind=RUNTIME_ACTIVATION_AUTHORIZATION_LEDGER_KIND,
        source_id=str(authorization.get("authorization_id")),
        status=RUNTIME_ACTIVATION_AUTHORIZATION_LEDGER_STATUS,
        compact=compact,
        ts=ts,
        latest_run=latest_run,
        schedule_item_id=schedule_item_id,
    ...
    decision_summary = compact.get("decision", {})
    ...
    return {
        ...
        "worker_step": {
            "status": "recorded_review_only",
            "worker_mode": "generation_runtime_activation_authorization_recorder",
            "schedule_item_id": schedule_item_id,
            "runtime_activation_authorization_cache_status": (
                RUNTIME_ACTIVATION_AUTHORIZATION_CACHE_STATUS
            ),
            "activation_decision": decision_summary.get("decision"),
            "developer_approval_recorded": (
                decision_summary.get("developer_approval_recorded") is True
            ),
            "target_count": int(decision_summary.get("target_count", 0) or 0),
            "provider_call_count": 0,
            "world_mutation_count": 0,
            "runtime_ready_count": 0,
            "activation_allowed_count": 0,
