...
- `ProviderAdapterRunner` 是工具层执行入口，不是后端自动后台执行器；默认 dry-run，显式 `llm_text` / `image` live 模式也必须只产出 `ProviderAdapterExecutionReceipt` 与 `ProviderOutputEnvelope`。image live 只能下载成本地 review-only artifact ref，不能保存临时 URL、provider 原始响应或直接晋升 published media。
