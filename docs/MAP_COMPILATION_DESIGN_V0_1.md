...
`MapComponentMediaManifest` 同样只是表现层组件媒体事实：它证明本地组件文件、尺寸、sha 和 style/node 归属，不证明地图玩法事实。当前 deterministic SVG baseline 会先进入 generation request pack，再进入 artifact staging 的 36 个 awaiting slot；candidate review 会读取该 staging manifest，但因为 `imported_count=0`，仍只保留 baseline fixture evidence；visual quality report 因无 generated candidate 保持 `awaiting_generated_candidates`，promotion gate 会显式读取该 visual report 并继续阻断晋升。approval tools 只能基于 alternate imported reports 派生 candidate-approved / visual-approved alternate evidence，让 promotion gate 证明受控链路可出现 `promotion_allowed_count > 0`；默认 demo / 正式 examples 仍保持 0 imported / 0 promoted。即使 manifest URL 可解析，前端默认 runtime 是否消费这些组件也必须由后续明确发布 / 前端合同任务决定。
