...
它不接入前端，不内嵌战斗 runtime package，也不会把候选资产自动晋升为默认可玩内容。它的目标是把当前 AI 编译系统从“单阶段样例”推进到“多阶段内容生产链”：连续生产 Stage 05 / Stage 06 / Stage 07 的世界线、玩家线、任务、随机事件、临时样本、研发任务和资产候选，交给项目负责人审查。当前 Stage 05 额外引用一个人工晋升后的旧信号塔压力战 runtime package，用来证明候选资产可以被封装进运行时证据包；这不等于世界线自动解锁最终蓝图。
...
当前多阶段样例的三个资产都应停在 `fallback_ready`：玩法核心可作为候选审查，但不能自动进入默认战斗主路径。Stage 05 的回光棱镜中继塔已有一个单独的 runtime package 引用，用于验证“候选资产 -> locked manifest -> battle config -> runtime package”的封装链路；Stage 06 / 07 仍只停在审查和 fallback 证据阶段。
...
- 后续核心对象迁移应落到每个阶段引用的 `WorldStateDelta`、`WorldStateDeltaTransaction`、runtime package，或具体 story asset review / promotion pack。

校验旧信号塔压力战 runtime package：
