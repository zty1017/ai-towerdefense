...
理论上，游戏中的塔、怪物、技能、素材、剧情、任务、随机事件、地图节点、经济规则和开发工具链都可以被抽象为 AI 可编译对象。但工程上必须区分“纳入编译体系”和“允许 AI 自由生成”：

- A 类，适合玩家受控自由编译：防御塔、技能、简单道具、表现资产、图标、局部描述、编译报告。
- B 类，适合系统辅助编译：怪物、素材、关卡环境、随机事件、NPC 建议、研究任务、肉鸽遗物。
- C 类，只适合开发者受控编译：经济系统、全局规则、核心战斗公式、寻路、存档、网络、底层代码、安全策略。

项目目标不是证明 AI 能无约束生成一切，而是证明游戏内容可以被统一抽象为 ObjectSpec，并通过 LLM + DAG + Validator 转化为可审查、可回滚、可运行的游戏实体或状态变化。

## 叙事型对象

剧情、任务和随机事件不是“文本对象”，而是状态机对象。它们至少应包含：

- 触发条件和前置条件。
- 参与 NPC、地图节点、材料、研究任务或资产引用。
- 展示内容，包括旁白、对话、事件说明。
- 目标链、选择、失败条件和完成条件。
- `state_delta` 或 `WorldStateDelta`，说明它实际改变了哪些运行态。
- 后续节点或依赖，避免分支无法收束。
- Validator 列表，包括设定一致性、任务可达性、奖励平衡、重复度、语气和运行时可执行性。
...
世界状态 / 战斗结果 / 玩家行为
  ...
  -> Context Retriever
  -> NarrativeObjectSpec
  -> Canon / WorldState Validator
  -> Quest Objective Compiler
  -> Reward / Balance Gate
  -> Dialogue / Presentation Generator
  -> Branch / Reachability Validator
  -> WorldStateDelta / StageCandidatePack / RunWorldState
```

这意味着“世界书演化”不能自由扩写，而应服务玩法和进度：生成任务、随机事件、NPC 建议、研究机会、材料约束、地图压力和后续可检测的状态变化。

## 素材与 NPC

素材不是普通背包物品，而是编译语言里的约束和能力 token。一个素材应表达它能支持哪些效果、带来哪些风险、属于什么稀有度、能否降低失败率或解锁特定机制。例如辉晶可以支持显形、减速、回光和侦测，但不应凭空支持高伤害爆破。

NPC 可以分为通用 NPC 与功能 NPC。通用 NPC 主要承担叙事、关系和世界感；功能 NPC 可以同时承担剧情角色和玩法接口，例如研究员、斥候、商人、工匠、医疗员。功能 NPC 的 AI 输出应以建议、评审、风险提示、可选方案和任务触发为主，不应直接越权修改核心规则。
