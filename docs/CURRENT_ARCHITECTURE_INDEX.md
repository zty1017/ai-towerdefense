...
- 地图 RenderPlan 后端只读接入：`backend/app/services/map_render_plan_service.py` 已把首战 `gray_lantern_station` 映射到 `MapStylePack`、`ProceduralMapRenderPlan` 和 `SemanticVisualConsistencyReport` bundle；`/api/sessions/{session_id}/battles/{node_id}/config` 与 `/runtime-package` 会可选返回 `map_render_plan_bundle`，`/map-render-plan` 提供单独读取入口。当前只有首战节点有 bundle，其他节点保持 `map_render_plan_bundle=null` 并在单独 endpoint 返回 404。
