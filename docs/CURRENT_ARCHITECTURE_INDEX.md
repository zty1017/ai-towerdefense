...
- `shared/schemas/map_component_candidate_review_report.v0.1.schema.json`、`tools/media/build_map_component_candidate_review_report.py`、`tools/media/validate_map_component_candidate_review_report.py`、`examples/review_packs/map_component_candidate_review_report.v0.1.json`
  - MapComponentCandidateReviewReport v0.1 是 artifact staging 之后的候选审查层。当前没有真实图像/视频生成候选，因此 36 个 deterministic SVG baseline 只被登记为 `baseline_fixture_candidate` / `no_generated_candidate_yet`，报告状态为 `blocked_from_promotion`，明确下一步需要本地生成 artifact、visual QA、cutout / normalization、binding refresh 和显式 promotion gate；SVG fixture 不得被当作真实 AI 生成结果。
- `shared/schemas/map_component_promotion_gate_report.v0.1.schema.json`、`tools/media/build_map_component_promotion_gate_report.py`、`tools/media/validate_map_component_promotion_gate_report.py`、`examples/review_packs/map_component_promotion_gate_report.v0.1.json`
  - MapComponentPromotionGateReport v0.1 是 candidate review 之后的显式晋升门。当前 generated candidates 为 0，`promotion_allowed_count=0`，`baseline_preserved_count=36`，并声明没有写入新的 MapComponentMediaManifest、StylePack、RenderPlan、frontend default 或 runtime map truth；只有未来 generated candidate 通过 review / QA / normalization / binding refresh / developer approval 后，才允许单独生成 manifest replacement 或新的 promotion report。
...
- Map component AI 生成候选与晋升门：统一 demo evidence 现在纳入 `MapComponentGenerationRequestPack -> MapComponentArtifactStagingManifest -> MapComponentCandidateReviewReport -> MapComponentPromotionGateReport`。当前 request pack 覆盖 36 个 baseline component；artifact staging 派生 36 个本地导入 slot，明确 `imported_count=0`、`awaiting_count=36`，且 staging 不等于 review passed；candidate review 明确 `generated_candidate_count=0`，只存在 baseline fixture candidate；promotion gate 明确 `promotion_allowed_count=0`、`baseline_preserved_count=36`。这条链路只为未来真实图像/视频组件生成提供 request -> local artifact staging -> review -> promotion 结构，不改变玩家默认前端、MapRuntimePackage、StylePack、RenderPlan 或现有 MapComponentMediaManifest。
