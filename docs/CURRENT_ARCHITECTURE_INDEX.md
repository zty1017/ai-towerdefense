...
- 前端默认战斗底座：`frontend/app.js` 当前用 `drawProceduralTerrain()`、`drawPlayableFieldBoundary()`、`drawPath()`、`drawRouteFlowCues()`、`drawBuildableTerraces()`、`drawDeploymentBase()`、`drawObjectiveDefensiveZone()`、`drawTargetFoundation()` 和 `drawSpawnRift()` 从 `MapRuntimePackage` 稳定生成玩家默认战场画面；`computeBattleMetrics()` 已按 runtime bounds 与 HUD safe area 做 contain fit，多路线敌人会绑定 runtime route / spawn id；整张 map image 只保留为 future published layer / debug evidence 语义，不进入默认 preload 或 `drawBackdrop()`。`tools/frontend/validate_battle_visual_contract.py` 已把这一点纳入静态合约。
