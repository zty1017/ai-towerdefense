...
- Generation Scheduler review-only pipeline smoke：`tools/dev/check_generation_scheduler_review_only_pipeline.py` 与 `examples/review_packs/generation_scheduler_review_only_pipeline_smoke_report.v0.1.json` 已作为本地 HTTP 闭环证据接入 `tools/demo/export_evidence.py` 的 `generation_scheduler.review_only_pipeline_smoke`。该 smoke 用临时 SQLite 和本地 uvicorn 走通 background handoff tick、queue / worker-cache / artifact-ledger / prefetch-cache / activation-gate 读模型、unsafe metadata 409、fixture executor chain、image_failure 负样本、shared cache 空命中和 reuse 409；它只证明 review-only scheduler pipeline、handoff outbox 和阻断边界可重复，不代表 live provider、真实图生视频、自动 staging / promotion、queue complete、runtime package 构建、WorldStateDeltaTransaction 写入、玩家侧发布或 runtime activation 已完成。
