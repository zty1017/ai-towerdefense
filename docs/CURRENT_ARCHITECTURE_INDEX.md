...
- MapStylePack component binding 审查门：`map_style_component_binding_report.v0.1` 已纳入统一 demo evidence，解析显式 `media:<stable_internal_id>` / `atlas:<animation_id>` 引用并记录 procedural fallback。当前示例报告为 `warning`，因为现有三份 StylePack 继续使用 procedural fallback，没有把 enemy / tower / NPC 媒体误当地图组件；这是 component coverage gap，不是 runtime activation。
