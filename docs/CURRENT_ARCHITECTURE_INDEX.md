...
- `tools/asset_graph/validation_common.py`
  - AssetGraph 校验 / 地图包工具共享 helper：集中 `load_json()`、`write_json()`、敏感字段扫描和可选 JSON Schema 校验。MapRuntimePackage v0.1 / v0.2 builder、MapCompilePackage builder 和对应 validator 已复用该 helper，保持原有 pretty JSON 输出顺序策略；`nodes.py` / workflow runner 这类核心 DAG 执行器暂时保留自身 artifact IO，以免混淆节点输出语义。
