...
- 地图补丁、补丁后 overlay 与拓扑重生：`tools/media/build_runtime_map_patch_candidates.py`、`examples/review_packs/runtime_map_patch_candidates.v0.1.json`、`tools/media/build_map_patch_overlay_review.py`、`examples/review_packs/map_patch_overlay_review.v0.1.json`、`tools/media/build_topology_constrained_map_prompt_pack.py`、`tools/media/generate_topology_constrained_map_candidates.py` 与 `examples/review_packs/topology_constrained_map_*.json` 已把布局修订计划拆成 review-only runtime 补丁、补丁后 overlay 复核和旧信号塔拓扑约束重生证据。下一步应迭代旧信号塔 prompt，降低塔体规模、禁止小人/杂物，并重新走 review/alignment/overlay/visual gate。
