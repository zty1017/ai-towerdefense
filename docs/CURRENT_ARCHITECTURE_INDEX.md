...
- `tools/media/build_map_runtime_promotion_readiness_report.py`、`tools/media/validate_map_runtime_promotion_readiness_report.py`、`examples/review_packs/map_runtime_promotion_readiness_report.v0.1.json`
  - 地图 runtime 晋升准备度读模型：聚合默认 `MapRuntimePackage v0.1`、`MapRuntimePackage v0.2 preview`、v0.2 RenderPlan、SemanticVisualConsistencyReport、MapCompilePackage 和地图视觉发布闸门，逐节点说明 v0.2 强语义已经形成 promotion candidate，但仍被显式 activation gate、发布图层 overlay correction 和 review-only/拒绝候选隔离要求阻断。该报告不调用 provider、不读取 `.env`、不写世界状态、不替换玩家默认地图。
- `tools/media/build_map_runtime_activation_gate_report.py`、`tools/media/validate_map_runtime_activation_gate_report.py`、`examples/review_packs/map_runtime_activation_gate_report.v0.1.json`
  - 地图 runtime 显式激活门：读取 readiness、v0.2 preview API smoke、视觉发布闸门和 v0.1/v0.2 地图包，逐节点给出 activation decision。当前三张节点均为 `blocked`，允许数为 0，阻断原因为缺少显式开发者激活批准、视觉 overlay correction、review-only/拒绝候选隔离、API/frontend 合同更新和激活后证据复跑。该报告是 gate/evidence，不修改默认 runtime、后端接口或前端行为。
...
MapRuntimePromotionReadinessReport v0.1 已接入 demo evidence，逐节点聚合 v0.1 默认 runtime、v0.2 preview、RenderPlan、语义一致性报告、MapCompilePackage 和视觉发布闸门。当前三张节点均为 `promotion_candidate_activation_required`：v0.2 强语义、RenderPlan 与语义一致性已满足候选条件，但 activation allowed 仍为 0，且还保留发布图层 overlay correction 与 review-only/拒绝候选隔离 blocker。因此它是 runtime 晋升读模型，不是激活命令。
MapRuntimeActivationGateReport v0.1 已接入 demo evidence，逐节点消费 readiness 与 preview API 证据并给出显式 activation decision。当前三张节点均为 `blocked`，允许激活数为 0；阻断项包括显式开发者激活批准缺失、发布图层 overlay correction、review-only/拒绝候选隔离、API/frontend 合同更新和激活后证据复跑。因此 v0.2 强语义当前被正式登记为“候选但不可激活”，默认玩家 runtime 继续保持 v0.1。
