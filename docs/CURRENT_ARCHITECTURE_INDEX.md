...
- 地图补丁、补丁后 overlay、拓扑重生与控制图：`tools/media/build_runtime_map_patch_candidates.py`、`examples/review_packs/runtime_map_patch_candidates.v0.1.json`、`tools/media/build_map_patch_overlay_review.py`、`examples/review_packs/map_patch_overlay_review.v0.1.json`、`tools/media/build_topology_constrained_map_prompt_pack.py`、`tools/media/generate_topology_constrained_map_candidates.py`、`tools/media/build_map_topology_control_sketch_pack.py` 与 `examples/review_packs/topology_constrained_map_*.json` / `examples/review_packs/map_topology_control_sketch_pack.v0.1.json` 已把布局修订计划拆成 review-only runtime 补丁、补丁后 overlay 复核、旧信号塔拓扑约束重生证据和三节点 topology control sketch。下一步应基于控制图/参考构图做受控重生或局部清理，而不是继续 prompt-only 抽图，并重新走 review/alignment/overlay/visual gate。
