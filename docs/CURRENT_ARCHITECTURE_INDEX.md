...
- `tools/dev/command_runner.py`、`tools/dev/quality_gate_report_helpers.py`
  - 本地 QA / evidence 脚本共享 helper：`command_runner.py` 被 `run_fast_quality_gate.py`、`run_demo_evidence_suite.py` 和 `export_evidence.py` 共用，用于处理 timeout、输出截断和时间戳；`quality_gate_report_helpers.py` 只服务 fast / premerge 本地质量门的命令失败收集、summary 推导、顶层状态推导、失败输出和 JSON report 写入。后续本地验收脚本应优先复用这些 helper，避免复制 subprocess 包装和 report summary 逻辑。
