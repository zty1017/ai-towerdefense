...
- 地图补丁、补丁后 overlay 与拓扑 prompt：`tools/media/build_runtime_map_patch_candidates.py`、`examples/review_packs/runtime_map_patch_candidates.v0.1.json`、`tools/media/build_map_patch_overlay_review.py`、`examples/review_packs/map_patch_overlay_review.v0.1.json`、`tools/media/build_topology_constrained_map_prompt_pack.py` 与 `examples/review_packs/topology_constrained_map_prompt_pack.v0.1.json` 已把布局修订计划拆成 review-only runtime 补丁、补丁后 overlay 复核和拓扑约束重生输入。下一步应对补丁后 overlay 做人工/视觉模型复核，或调用真实图像 provider 生成旧信号塔拓扑约束候选，然后重新走 promotion gate。
