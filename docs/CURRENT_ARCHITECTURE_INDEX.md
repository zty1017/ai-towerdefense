...
RuntimeMapPatchCandidates v0.1 已接入 demo evidence，为灰灯驿站和灯芯仓产出 review-only 坐标/路径补丁候选，旧信号塔因拓扑冲突跳过补丁并转入拓扑约束重生。TopologyConstrainedMapPromptPack v0.1 已接入 demo evidence，包含旧信号塔主重生 prompt，以及灰灯驿站、灯芯仓的 fallback prompt。两者都不直接修改 MapRuntimePackage 或发布视觉层。
...
- 地图补丁与拓扑 prompt：`tools/media/build_runtime_map_patch_candidates.py`、`examples/review_packs/runtime_map_patch_candidates.v0.1.json`、`tools/media/build_topology_constrained_map_prompt_pack.py` 与 `examples/review_packs/topology_constrained_map_prompt_pack.v0.1.json` 已把布局修订计划拆成两类 review-only 输入。下一步应渲染 patch 后 overlay 或调用真实图像 provider 生成拓扑约束候选，然后重新走 review gate。
