...
- `tools/provider_adapter/run_provider_adapter.py`
  - Provider adapter runner 工具层事实源：默认 fixture dry-run，不读取 `.env`、不调用 provider；显式 `--mode llm_text --live` 才允许调用 LLM adapter，且仍只能输出 ProviderAdapterExecutionReceipt、ProviderOutputEnvelope 和 redacted summary refs。
