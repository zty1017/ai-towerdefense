...
- Demo evidence suite 的 schema、suite id、报告文件名和子命令名已集中到 `tools/demo/demo_evidence_suite_contract.py`，runner / validator / 质量门不应再复制硬编码。
- 已补日常开发快速质量门 `tools/dev/run_fast_quality_gate.py`、`tools/dev/fast_quality_gate_contract.py` 与 report validator：串联 Python 编译、前端语法检查、战斗视觉合同、战斗交互合同、campaign router 前端合同、map component 前端合同、WorkerTaskPack runner 前置 env 执行 smoke、WorkerTaskPack acceptance profile 只读审计、release gate profile 降级审计、MVP readiness build 和 readiness validator，默认输出 `/tmp/ai_td_fast_quality_gate_report.v0.1.json`；contract 模块统一保存 schema、report id、命令顺序和安全边界常量，runner 复用 `tools/dev/quality_gate_report_helpers.py` 推导 summary、顶层状态和失败输出，写出报告后自校验，`tools/dev/validate_fast_quality_gate_report.py` 仍可独立校验 schema、命令顺序、summary 计数、runner env smoke、WorkerTaskPack profile audit、release gate audit 和安全边界。它不跑浏览器、不调用 provider、不读取 `.env`、不写世界状态、不激活 runtime，用于比完整 evidence export 更快地发现常见破坏；录屏 / 评审前仍以完整 evidence 套件为准。
- 已补本地合并前质量门 `tools/dev/run_premerge_quality_gate.py`、`tools/dev/premerge_quality_gate_contract.py` 与 report validator：默认 profile 复用 fast gate、fast gate report validator、WorkerTaskPack 全量 dry-run、batch report validator、profile 审计、release gate 降级审计、迁移 dry-run 和 `git diff --check`，不跑浏览器、不调用 provider、不读取 `.env`；contract 模块统一保存 profile、schema、report id、命令顺序、必需命令集合和安全边界常量，runner 复用 `tools/dev/quality_gate_report_helpers.py` 推导 summary、顶层状态和失败输出，写出报告后自校验，validator 仍可独立校验 report id、release gate audit、完整命令顺序 / fail-fast 前缀和 no-provider / no-env / no-runtime 边界；可选 full profile 追加默认完整 evidence export。
- 已补本地 dev report 状态 contract 与 WorkerTaskPack acceptance profile runner：`tools/dev/report_status_contract.py` 集中本地 dev report 共享状态枚举，`tools/dev/worker_acceptance_report_contract.py` 作为 worker acceptance 命名空间下的兼容 re-export，`tools/dev/worker_acceptance_profile_contract.py` 集中单包 profile report schema 与默认输出路径；`tools/dev/run_worker_acceptance_profile.py` 先校验任务包，再按 `acceptance_profile.default_profile` 或显式 `--profile` 安全执行命令，支持 profile 列表、dry-run、fail-fast、timeout 和 JSON report。该 runner 不使用 shell，支持前置 `KEY=value` token 并会传入子进程环境，避免 `PYTHONPYCACHEPREFIX` 丢失后写仓库 `__pycache__`；遇到管道、非受限重定向、分号连接、逻辑连接或命令替换语法会记录 unsupported 并拒绝执行；最终 token 形式的 `> /tmp/file` 或 `>/tmp/file` 会由 runner 捕获 stdout 后写入仓库外 `/tmp` 文件；没有 `acceptance_profile` 的旧包仍需手动运行 `acceptance_commands`。
- 已补 WorkerTaskPack acceptance profile 批量 contract、runner 与 batch report validator：`tools/dev/worker_acceptance_batch_contract.py` 集中 batch report schema 与默认输出路径，并复用共享状态枚举；`tools/dev/run_worker_acceptance_batch.py` 支持显式 `--task-pack`、`--task-id-prefix`、`--path-contains`、`--all`、`--profile`、`--dry-run`、`--fail-fast` 和 JSON report，默认拒绝隐式全量选择；全量 dry-run 用于快速确认任务包 profile 可解析，日常真实执行仍应缩小到明确任务包或筛选条件。
...
- `tools/frontend/validate_battle_interaction_contract.py` 已用于无浏览器环境的静态战斗交互合约校验，锁定工具卡拖拽部署主路径、点击放置 fallback、战场预览、拖拽 ghost 和移动端 pointer/touch 基线。
...
- `tools/frontend/validate_battle_interaction_contract.py` 已把拖拽部署主路径纳入 fast / premerge gate。
...
- `tools/dev/run_fast_quality_gate.py`：实际运行 env assignment smoke，让日常 fast gate 能发现前置 env 执行回归；`tools/dev/run_premerge_quality_gate.py` 通过 fast gate 自动继承该检查。
...
### P1-E-4b WorkerTaskPack release gate profile downgrade audit

状态：已完成只读降级审计并接入 fast/premerge gate。

目标：

```text
扫描 WorkerTaskPack 的 release_gate profile，阻止录屏 / 发布候选验收命令混入日常快速反馈使用的降级开关，避免最终评审证据被误配置。
```

已落地：

- `tools/dev/audit_release_gate_profiles.py`：新增只读审计 CLI，默认扫描 `examples/worker_task_packs`，默认输出 `/tmp/release_gate_profile_audit_report.v0.1.json`，复用任务包 validator 与 profile runner 命令解析。
- `tools/dev/run_fast_quality_gate.py`：将 release gate 降级审计纳入日常快速质量门。
- `tools/dev/run_premerge_quality_gate.py`：将 release gate 降级审计纳入合并前质量门。
- `examples/worker_task_packs/p1e_release_gate_profile_audit.v0.1.json`：新增任务包，记录本工具的边界与验收。
- `examples/worker_task_packs/p1e_worker_acceptance_command_profiles.v0.1.json`：修正 `release_gate` 中 demo suite 缺少 suite validator 的示例命令。
- `docs/WORKER_TASK_PACK_V0_1.md`、`docs/CURRENT_ARCHITECTURE_INDEX.md`：补充 release gate 降级规则和事实源索引。

边界：

- 审计不执行任何被扫描任务包里的 `release_gate`、`daily_fast` 或 `acceptance_commands`。
- 没有 `release_gate` 的任务包不会失败；工具只检查已经声明 release gate 的任务包。
- `release_gate` 不允许 `--allow-missing-browser`、`--allow-browser-unavailable`、`--validation-profile summary-only` 或通用 scheduler/outbox runner mode 锁定。
- 如果 `release_gate` 运行 `run_demo_evidence_suite.py`，必须配套 `validate_demo_evidence_suite_report.py --require-browser-captured`，并保持 scheduler / outbox smoke 断言。
- 本任务不修改 `backend/`、`frontend/`、`examples/review_packs/`、`game_data/`、`.env` 或默认 runtime/package 文件。
...
```text
减少录屏 / 评审前反复运行 demo evidence suite 的等待时间：scheduler pipeline smoke 默认优先复用当前 worktree 或 git common dir 对应主工作区的 .venv/bin/python；没有可用 .venv 的干净环境 / CI 继续使用 uv run 回退。
```

已落地：

- `tools/demo/run_demo_evidence_suite.py`：新增 `--scheduler-smoke-runner {auto,uv,venv,current-python}` 与 `--scheduler-python`。
- 默认 `auto` 在当前 worktree `.venv/bin/python` 或共享主工作区 `.venv/bin/python` 存在时使用本地 venv；都不存在时保持原来的 `uv run --extra dev` 隔离执行。
...
### P1-D-38 Demo evidence suite multinode visual smoke
...
```text
把 P1-D-37 的三节点战斗视觉矩阵纳入录屏 / 评审前默认 suite，减少演示前手动命令遗漏。
```

已落地：

- `tools/demo/run_demo_evidence_suite.py`：在玩家主链路截图后默认运行 `frontend_multinode_visual_smoke_capture` 和 `frontend_multinode_visual_smoke_validate`。
- suite report 新增 `frontend_multinode_report` 输出引用和 `frontend_multinode_visual_smoke` 摘要，记录 3 节点 x 2 视口的截图矩阵、截图数量、失败数和安全计数。
- `tools/demo/validate_demo_evidence_suite_report.py`：`--require-browser-captured` 现在同时要求 14 张玩家主链路截图和 6 张多节点战斗截图。
- `README.md`、`docs/MVP_REVIEW_HANDOFF_V0_1.md`、`docs/CURRENT_ARCHITECTURE_INDEX.md`：同步一键套件说明。
- `examples/worker_task_packs/p1d_demo_evidence_suite_runner.v0.1.json`：同步任务包边界和验收说明。

边界：

- 不调用 provider、不读取 `.env`、不生成素材、不写世界状态、不提交截图、不修改 evidence export 的 readiness 输入。
- `export_evidence.py` 仍只消费玩家主链路 report；多节点 report 是 suite 级视觉回归证据，不伪装成 MVP readiness gate。
- 无浏览器环境仍必须显式 `--allow-missing-browser`，并在 suite report 中保留两个 browser_unavailable 证据。
...
python3 tools/demo/validate_demo_evidence_suite_report.py /tmp/battle_drag_demo_suite_full/demo_evidence_suite_report.v0.1.json --allow-browser-unavailable --require-scheduler-pipeline-smoke --require-outbox-import-smoke
git diff --check
```
...
### P1-E-15b FastQualityGateReportValidator v0.1
...
```text
新增 fast quality gate 结构化报告 validator，并把它接入 premerge gate，避免日常 fast gate 只凭命令退出码判断成功，报告 schema / 命令顺序 / 安全边界漂移却未被发现。
```
...
- `examples/worker_task_packs/p1e_fast_quality_gate_report_validator.v0.1.json`：固化本轮 daily 验收。
- `examples/worker_task_packs/p1e_premerge_quality_gate.v0.1.json`、`docs/CURRENT_ARCHITECTURE_INDEX.md`、`control/TASK_QUEUE.md`：同步本地质量门事实源。

边界：

- 不调用 provider、不读取 `.env`、不跑浏览器、不写世界状态、不激活 runtime。
- validator 只读取 fast gate 输出 JSON；它不重新执行 fast gate，也不替代 demo evidence suite 或人工最终审查。
- passing fast report 默认要求当前完整命令顺序，后续修改 fast gate 命令列表时必须同步 validator 和任务包。
...
### P1-E-15d FastGateContractModule v0.1

状态：已完成 fast gate 契约常量收敛。

目标：

```text
把 fast quality gate 的 schema、report id、命令顺序和安全边界常量收敛到共享 contract 模块，减少 runner 与 report validator 的重复维护，同时保留独立命令契约检查。
```

已落地：

- `tools/dev/fast_quality_gate_contract.py`：新增 fast gate 本地契约常量，包括 report schema、report id、13 个命令名、命令顺序、安全边界字段和零计数字段。
- `tools/dev/run_fast_quality_gate.py`：命令名、schema version 和 report id 改为读取 contract 常量，并把 contract 纳入自身 `py_compile` 覆盖。
- `tools/dev/validate_fast_quality_gate_report.py`：移除重复硬编码命令列表，改为读取 contract；关键 smoke / audit 必需项仍独立按命令名检查。
- `tools/dev/run_premerge_quality_gate.py`：把 contract 纳入 premerge 编译覆盖。
- `examples/worker_task_packs/p1e_fast_gate_contract_module.v0.1.json`：固化本轮验收。
- `examples/worker_task_packs/p1e_fast_quality_gate_report_validator.v0.1.json`、`examples/worker_task_packs/p1e_fast_gate_worker_profile_audit.v0.1.json`、`examples/worker_task_packs/p1e_premerge_quality_gate.v0.1.json`、`docs/CURRENT_ARCHITECTURE_INDEX.md`、`control/TASK_QUEUE.md`：同步 contract 事实源。

边界：

- 不改变 fast gate 执行命令集合，不放宽 report validator。
- validator 仍按 contract 检查 runner 产出的完整命令顺序；不是从 report 自身反推。
- 不调用 provider、不读取 `.env`、不跑浏览器、不写世界状态、不激活 runtime。

验收：

```bash
python3 tools/dev/validate_worker_task_pack.py examples/worker_task_packs/p1e_fast_gate_contract_module.v0.1.json
python3 tools/dev/validate_worker_task_pack.py examples/worker_task_packs/p1e_fast_quality_gate_report_validator.v0.1.json
python3 tools/dev/validate_worker_task_pack.py examples/worker_task_packs/p1e_fast_gate_worker_profile_audit.v0.1.json
python3 tools/dev/validate_worker_task_pack.py examples/worker_task_packs/p1e_premerge_quality_gate.v0.1.json
PYTHONPYCACHEPREFIX=/tmp/ai_td_pycache_fast_gate_contract_module python3 -m py_compile tools/dev/fast_quality_gate_contract.py tools/dev/run_fast_quality_gate.py tools/dev/validate_fast_quality_gate_report.py tools/dev/run_premerge_quality_gate.py tools/dev/command_runner.py
python3 tools/dev/run_fast_quality_gate.py --output /tmp/fast_gate_contract_module_fast_report.json
python3 tools/dev/validate_fast_quality_gate_report.py /tmp/fast_gate_contract_module_fast_report.json --expect-status passed --expect-failed-count 0 --require-worker-env-smoke --require-worker-profile-audit --require-release-gate-audit --require-complete-command-order
python3 tools/dev/run_premerge_quality_gate.py --output /tmp/fast_gate_contract_module_premerge_report.json --fail-fast
python3 tools/dev/validate_premerge_quality_gate_report.py /tmp/fast_gate_contract_module_premerge_report.json --expect-status passed --expect-profile premerge --expect-failed-count 0
git diff --check
```

### P1-E-15e PremergeGateContractModule v0.1

状态：已完成 premerge gate 契约常量收敛。

目标：

```text
把 premerge quality gate 的 schema、profile、命令名、命令顺序、必需命令集合和安全边界常量收敛到共享 contract 模块，减少 runner 与 report validator 的重复维护，并补上 release gate audit 为 premerge validator 必需命令。
```

已落地：

- `tools/dev/premerge_quality_gate_contract.py`：新增 premerge gate 本地契约常量，包括 report schema、report id、profile、命令名、命令顺序、必需命令集合、安全边界字段和零计数字段。
- `tools/dev/run_premerge_quality_gate.py`：命令名、profile、schema、report id、summary 零计数和 boundary 字段改为读取 contract 常量，并把 contract 纳入自身 `py_compile` 覆盖。
- `tools/dev/validate_premerge_quality_gate_report.py`：移除重复硬编码 profile / required command / boundary 列表，改为读取 contract；新增 report id 校验，把 `release_gate_profile_audit` 纳入必需命令集合，并校验完整命令顺序 / fail-fast 失败前缀。
- `tools/dev/run_fast_quality_gate.py`：把 premerge contract 纳入快速编译覆盖。
- `examples/worker_task_packs/p1e_premerge_gate_contract_module.v0.1.json`：固化本轮验收。
- `examples/worker_task_packs/p1e_premerge_quality_gate.v0.1.json`、`examples/worker_task_packs/p1e_fast_gate_contract_module.v0.1.json`、`examples/worker_task_packs/p1e_fast_quality_gate_report_validator.v0.1.json`、`examples/worker_task_packs/p1e_fast_gate_worker_profile_audit.v0.1.json`、`docs/CURRENT_ARCHITECTURE_INDEX.md`、`control/TASK_QUEUE.md`：同步 contract 事实源。

边界：

- 不改变 premerge 执行命令集合，不放宽 report validator。
- validator 仍按 contract 检查 runner 产出的报告；release gate audit 是合并前必需命令，命令顺序由 contract 固化。
- 不调用 provider、不读取 `.env`、不跑浏览器、不写世界状态、不激活 runtime。

验收：

```bash
python3 tools/dev/validate_worker_task_pack.py examples/worker_task_packs/p1e_premerge_gate_contract_module.v0.1.json
python3 tools/dev/validate_worker_task_pack.py examples/worker_task_packs/p1e_premerge_quality_gate.v0.1.json
python3 tools/dev/validate_worker_task_pack.py examples/worker_task_packs/p1e_fast_gate_contract_module.v0.1.json
PYTHONPYCACHEPREFIX=/tmp/ai_td_pycache_premerge_gate_contract_module python3 -m py_compile tools/dev/premerge_quality_gate_contract.py tools/dev/run_premerge_quality_gate.py tools/dev/validate_premerge_quality_gate_report.py tools/dev/run_fast_quality_gate.py tools/dev/fast_quality_gate_contract.py tools/dev/validate_fast_quality_gate_report.py tools/dev/command_runner.py
python3 tools/dev/run_premerge_quality_gate.py --output /tmp/premerge_gate_contract_module_premerge_report.json --fail-fast
python3 tools/dev/validate_premerge_quality_gate_report.py /tmp/premerge_gate_contract_module_premerge_report.json --expect-status passed --expect-profile premerge --expect-failed-count 0
python3 tools/dev/run_fast_quality_gate.py --output /tmp/premerge_gate_contract_module_fast_report.json
python3 tools/dev/validate_fast_quality_gate_report.py /tmp/premerge_gate_contract_module_fast_report.json --expect-status passed --expect-failed-count 0 --require-worker-env-smoke --require-worker-profile-audit --require-release-gate-audit --require-complete-command-order
git diff --check
```

### P1-E-15f PremergeGateCommandOrderContract v0.1

状态：已完成 premerge gate 命令顺序契约加固。

目标：

```text
让 premerge quality gate 像 fast gate 一样由共享 contract 明确定义命令顺序，并让 report validator 校验完整顺序或 fail-fast 失败前缀，避免合并前质量门发生隐性漂移。
```

已落地：

- `tools/dev/premerge_quality_gate_contract.py`：新增 `PREMERGE_COMMAND_ORDER` 与 `FULL_COMMAND_ORDER`，并由它们派生 required command set。
- `tools/dev/validate_premerge_quality_gate_report.py`：新增命令顺序校验；通过或非 fail-fast 报告必须匹配完整顺序，fail-fast 失败报告必须匹配 contract 顺序前缀。
- `examples/worker_task_packs/p1e_premerge_gate_command_order_contract.v0.1.json`：固化本轮验收。
- `examples/worker_task_packs/p1e_premerge_gate_contract_module.v0.1.json`、`docs/CURRENT_ARCHITECTURE_INDEX.md`、`control/TASK_QUEUE.md`：同步 premerge contract 事实源。

边界：

- 不改变 premerge runner 的执行命令集合，不放宽 report validator。
- 不调用 provider、不读取 `.env`、不跑浏览器、不写世界状态、不激活 runtime。

验收：

```bash
python3 tools/dev/validate_worker_task_pack.py examples/worker_task_packs/p1e_premerge_gate_command_order_contract.v0.1.json
python3 tools/dev/validate_worker_task_pack.py examples/worker_task_packs/p1e_premerge_gate_contract_module.v0.1.json
PYTHONPYCACHEPREFIX=/tmp/ai_td_pycache_premerge_gate_command_order_contract python3 -m py_compile tools/dev/premerge_quality_gate_contract.py tools/dev/validate_premerge_quality_gate_report.py tools/dev/run_premerge_quality_gate.py tools/dev/run_fast_quality_gate.py tools/dev/validate_fast_quality_gate_report.py tools/dev/command_runner.py
python3 tools/dev/run_premerge_quality_gate.py --output /tmp/premerge_gate_command_order_contract_premerge_report.json --fail-fast
python3 tools/dev/validate_premerge_quality_gate_report.py /tmp/premerge_gate_command_order_contract_premerge_report.json --expect-status passed --expect-profile premerge --expect-failed-count 0
python3 tools/dev/run_fast_quality_gate.py --output /tmp/premerge_gate_command_order_contract_fast_report.json
python3 tools/dev/validate_fast_quality_gate_report.py /tmp/premerge_gate_command_order_contract_fast_report.json --expect-status passed --expect-failed-count 0 --require-worker-env-smoke --require-worker-profile-audit --require-release-gate-audit --require-complete-command-order
git diff --check
```

### P1-E-15g QualityGateRunnerSelfValidation v0.1

状态：已完成质量门 runner 报告自校验。

目标：

```text
让 fast/premerge quality gate runner 在写出结构化报告后立即调用各自独立 validator 做自校验，减少日常开发时漏跑 report validator 的风险，同时保留独立 validator 作为任务包和 CI 复核入口。
```

已落地：

- `tools/dev/run_fast_quality_gate.py`：写出 report 后调用 `validate_fast_quality_gate_report.validate_report()` 自校验；命令失败仍按原逻辑返回失败，自校验失败也返回失败。
- `tools/dev/run_premerge_quality_gate.py`：写出 report 后调用 `validate_premerge_quality_gate_report.validate_report()` 自校验。
- `tools/dev/validate_fast_quality_gate_report.py`：补齐 fail-fast 失败报告的命令顺序前缀校验，与 premerge validator 口径一致。
- `README.md`、`docs/CURRENT_ARCHITECTURE_INDEX.md`、`control/TASK_QUEUE.md`：同步“runner 自校验，独立 validator 仍可复核”的事实源。
- `examples/worker_task_packs/p1e_quality_gate_runner_self_validation.v0.1.json`：固化本轮验收。

边界：

- 不改变 fast/premerge 执行命令集合，不放宽 report validator。
- 不调用 provider、不读取 `.env`、不跑浏览器、不写世界状态、不激活 runtime。
