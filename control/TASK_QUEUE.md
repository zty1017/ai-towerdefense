...
- 必须消费 `RuntimeMapPatchCandidates v0.1`、`TopologyConstrainedMapPromptPack v0.1/v0.2` 和 `MapTopologyControlSketchPack v0.1`。下一步候选任务：对 runtime patch candidate 重新生成 overlay PNG 复核；用 topology control sketch 作为参考构图调用真实图像 provider 或做局部清理，并重新走 candidate / alignment / overlay / visual review gates。
...
3. 地图补丁后 overlay 人工/视觉模型复核，以及基于 MapTopologyControlSketchPack 的参考构图受控重生 / 局部清理路线；只有通过 promotion gate 后才允许更新正式 MapRuntimePackage 或发布底图。
4. `P1-A` 真实视频关键帧增强。
